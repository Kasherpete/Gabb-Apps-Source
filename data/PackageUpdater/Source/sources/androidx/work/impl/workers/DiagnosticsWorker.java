...
public class DiagnosticsWorker extends Worker {
    ...
    private static String workSpecRows(WorkNameDao workNameDao, WorkTagDao workTagDao, SystemIdInfoDao systemIdInfoDao, List<WorkSpec> list) {
        ...
        for (WorkSpec next : list) {
            Integer num = null;
            SystemIdInfo systemIdInfo = systemIdInfoDao.getSystemIdInfo(next.f92id);
            if (systemIdInfo != null) {
                num = Integer.valueOf(systemIdInfo.systemId);
            }
            sb.append(workSpecRow(next, TextUtils.join(WebViewLogEventConsumer.DDTAGS_SEPARATOR, workNameDao.getNamesForWorkSpecId(next.f92id)), num, TextUtils.join(WebViewLogEventConsumer.DDTAGS_SEPARATOR, workTagDao.getTagsForWorkSpecId(next.f92id))));
        }
        return sb.toString();
    }

    private static String workSpecRow(WorkSpec workSpec, String str, Integer num, String str2) {
        return String.format("\n%s\t %s\t %s\t %s\t %s\t %s\t", new Object[]{workSpec.f92id, workSpec.workerClassName, num, workSpec.state.name(), str, str2});
    }
}
