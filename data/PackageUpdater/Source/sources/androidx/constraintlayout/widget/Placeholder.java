...
public class Placeholder extends View {
    ...
    private void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, C0166R.styleable.ConstraintLayout_placeholder);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0166R.styleable.ConstraintLayout_placeholder_content) {
                    this.mContentId = obtainStyledAttributes.getResourceId(index, this.mContentId);
                } else if (index == C0166R.styleable.ConstraintLayout_placeholder_placeholder_emptyVisibility) {
    ...
    public void onDraw(Canvas canvas) {
        if (isInEditMode()) {
            ...
            canvas.drawText(CoreFeature.DEFAULT_APP_VERSION, ((((float) width) / 2.0f) - (((float) rect.width()) / 2.0f)) - ((float) rect.left), ((((float) height) / 2.0f) + (((float) rect.height()) / 2.0f)) - ((float) rect.bottom), paint);
        }
    }
