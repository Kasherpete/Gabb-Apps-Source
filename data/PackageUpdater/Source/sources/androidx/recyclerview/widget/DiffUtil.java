...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake snake, Snake snake2) {
            int i = snake.f80x - snake2.f80x;
            return i == 0 ? snake.f81y - snake2.f81y : i;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (diffPartial != null) {
                ...
                diffPartial.f80x += range.oldListStart;
                diffPartial.f81y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (diffPartial.reverse) {
                    range2.oldListEnd = diffPartial.f80x;
                    range2.newListEnd = diffPartial.f81y;
                } else if (diffPartial.removal) {
                    range2.oldListEnd = diffPartial.f80x - 1;
                    range2.newListEnd = diffPartial.f81y;
                } else {
                    range2.oldListEnd = diffPartial.f80x;
                    range2.newListEnd = diffPartial.f81y - 1;
                }
                arrayList2.add(range2);
                if (!diffPartial.reverse) {
                    range.oldListStart = diffPartial.f80x + diffPartial.size;
                    range.newListStart = diffPartial.f81y + diffPartial.size;
                } else if (diffPartial.removal) {
                    range.oldListStart = diffPartial.f80x + diffPartial.size + 1;
                    range.newListStart = diffPartial.f81y + diffPartial.size;
                } else {
                    range.oldListStart = diffPartial.f80x + diffPartial.size;
                    range.newListStart = diffPartial.f81y + diffPartial.size + 1;
                }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (snake == null || snake.f80x != 0 || snake.f81y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f80x + snake.size;
                int i4 = snake.f81y + snake.size;
                ...
                for (int i5 = 0; i5 < snake.size; i5++) {
                    int i6 = snake.f80x + i5;
                    int i7 = snake.f81y + i5;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i6 = snake.f80x + snake.size;
                int i7 = snake.f81y + snake.size;
                int i8 = 8;
                if (z) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                Snake snake = this.mSnakes.get(size);
                int i3 = snake.size;
                int i4 = snake.f80x + i3;
                int i5 = snake.f81y + i3;
                if (i4 < i) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f80x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f80x + i6, 1, this.mCallback.getChangePayload(snake.f80x + i6, snake.f81y + i6));
                    }
                }
                i = snake.f80x;
                i2 = snake.f81y;
