...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f64mX;

    /* renamed from: mY */
    private final float[] f65mY;

    PathInterpolatorApi14(Path path) {
        PathMeasure pathMeasure = new PathMeasure(path, false);
        float length = pathMeasure.getLength();
        int i = ((int) (length / 0.002f)) + 1;
        this.f64mX = new float[i];
        this.f65mY = new float[i];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < i; i2++) {
            pathMeasure.getPosTan((((float) i2) * length) / ((float) (i - 1)), fArr, (float[]) null);
            this.f64mX[i2] = fArr[0];
            this.f65mY[i2] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f64mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f64mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f64mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f65mY[i];
        }
        float[] fArr2 = this.f65mY;
        float f3 = fArr2[i];
